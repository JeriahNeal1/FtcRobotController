    ...
    public void loop() {
        ...
        if (yReset) {
            ...
            if (!claw_rolling) {
                ...
                double newPosition = currentPosition + (joystickX * 0.05); // Scale factor to make movement more controlled
                
                // Clamp position between 0 and 1 (servo valid range)
                newPosition = Math.min(Math.max(newPosition, 0.0), 1.0);
                
                // Set direction based on joystick direction
                if (joystickX > 0) {
                    clawRoll.setDirection(Servo.Direction.FORWARD);
                } else {
